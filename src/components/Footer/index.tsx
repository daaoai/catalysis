...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="w-full font-generalsans bg-white">
            ...
            {Object.entries(FooterContent.navigation).map(
              ([section, links]) => (
                <div key={section} className="flex flex-col space-y-4">
                  <h3 className="text-[#323131] dark:text-gray font-bold font-sans tracking-tight text-sm ">
                    {section}
                  </h3>
                  <ul className="space-y-1">
                    {links.map((link) => (
                      <li key={link.label}>
                        <Link
                          href={link.url}
                          className="text-gray-700 font-generalsans dark:text-white-40 hover:text-gray-900 dark:hover:text-white text-[14px] font-normal"
                        >
                          {link.label}
                        </Link>
                      </li>
